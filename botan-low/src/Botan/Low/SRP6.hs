...
srp6ServerSessionStep2 srp6 a =
    ...
    createWithSize (fromIntegral aLen) $ \ outPtr outLen ->
    throwBotanIfNegative_ $
...
-- @verifier[]@ buffer in the 'botan_srp6_generate_verifier' function. In
-- @botan-low@, such buffers are created internally using 'srp6GroupSize',
-- without requiring input from you (the user).
srp6GroupSize ::
