...
privKeyCreate
    :: ByteString
    -- ^ __algo_name__: something like "RSA" or "ECDSA"
    -> ByteString
    -- ^ __algo_params__: is specific to the algorithm. For RSA, specifies
    --   the modulus bit length. For ECC is the name of the curve.
    -> RNG
    -- ^ __rng__: a random number generator
    -> IO PrivKey
    -- ^ __key__: the new object will be placed here
privKeyCreate name params rng = asCString name $ \namePtr -> do
...
withPrivKeyCreate
    :: ByteString -> ByteString -> RNG -> (PrivKey -> IO a) -> IO a
...
privKeyLoad bits password = asBytesLen bits $ \bitsPtr bitsLen -> do
    let asCStringNullable str act =
            if ByteString.null str
                then act nullPtr
